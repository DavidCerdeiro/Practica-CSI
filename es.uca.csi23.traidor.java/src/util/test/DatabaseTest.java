...
class DatabaseTest {
	...
	void testDbAccess() throws IOException, SQLException {
		...
		try {
			...
			rs = con.createStatement().executeQuery("SELECT name FROM stadium ORDER BY name ASC;");
			
			int i = 0;
			while (rs.next()) {
				if(i == 0) assertEquals("Estadio de las Hadas futboleras", rs.getString("name"));
				if(i == 2) assertEquals("Ogro Stadium", rs.getString("name"));
				i++;
			}
